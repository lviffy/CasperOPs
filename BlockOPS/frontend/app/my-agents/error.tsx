...
export default function MyAgentsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    ...
    if (Sentry && typeof Sentry.captureException === "function") {
      Sentry.captureException(error, { tags: { boundary: "my-agents" } })
    } else {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center space-y-5">
        ...
        <div>
          <h1 className="text-xl font-semibold">Couldn&rsquo;t load your agents</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Your agents are safe in Supabase — this is just a render failure.
          </p>
          {error.digest && (
