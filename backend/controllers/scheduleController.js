/**
 * Schedule Controller — One-time and recurring on-chain transfers on Casper
 *
 ...
 *   amount           — human-readable CSPR amount (e.g. "10")
 *   tokenAddress     — optional (ignored for native CSPR transfer)
...
/**
 * Execute the actual transfer for a scheduled job on Casper.
 * Writes outcome back to Supabase.
 */
async function runTransfer(job) {
  const { id, private_key, to_address, amount } = job;
  
  let txHash = null;
  let error  = null;

  try {
    const keys = getKeysFromHex(private_key);
    if (!keys) {
    ...
    if (!compliant) {
      throw new Error(`Payer address ${fromAddress} is not compliant on the Compliance contract`);
    }
    ...
    if (BigInt(balanceMotes) < amountInMotes) {
      throw new Error(`Insufficient balance. Balance: ${(Number(balanceMotes) / 1_000_000_000).toString()} CSPR, Required: ${amount.toString()} CSPR`);
    }
    ...
    const session = DeployUtil.ExecutableDeployItem.newTransfer(
      ...
      CLPublicKey.fromHex(to_address),
      undefined,
      12345
    );
    ...
    txHash = await client.deploy(signedDeploy);

    console.log(`[Schedule] Job ${id} executed successfully on Casper. Tx: ${txHash}`);

    // Fire webhook as best-effort
    try {
...
async function createSchedule(req, res) {
  try {
    ...
    // Validate toAddress format (Casper public key hex)
    try {
      CLPublicKey.fromHex(toAddress);
    } catch (e) {
      return res.status(400).json(errorResponse('Invalid toAddress (must be a valid Casper public key hex)'));
    }
    ...
    if (!keys) {
      return res.status(400).json(errorResponse('Invalid privateKey'));
    }
    const walletAddress = keys.publicKey.toHex();
...
async function listSchedules(req, res) {
  try {
    ...
    let query = supabase
      .from('scheduled_transfers')
      .select('id, agent_id, type, cron_expression, to_address, amount, token_address, chain, wallet_address, status, run_count, last_run_at, last_tx_hash, last_error, created_at, label')
